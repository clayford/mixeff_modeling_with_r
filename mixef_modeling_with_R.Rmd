...
Technically model lmm1 is _nested_ within lmm2. It's a version of lmm2 where the interaction is 0. We can use the base R `anova()` function to compare the models using a hypothesis test. 
...
The lmm2 model allowed for one random effect on the intercept. Let's fit a new model that allows a random effect on weeks as well. We can do that by specifying `(weeks | subject)`. This says the effect of weeks is conditional on each rat. (This may not be correct.) By default this will also estimate subject random effect on the intercept!
...
The 95% confidence interval on the weeks random effect is about [2.6, 4.8]. I think we can safely conclude the random effect is not 0.

Answering this question with a hypothesis test is tricky, because it means testing if _variance equals 0_. This is a weird test since variance cannot be less than 0. We're at the boundary of possible values. This means it's not appropriate to use the hypothesis test implemented in the `anova()` function (though it will still run and provide results.)
