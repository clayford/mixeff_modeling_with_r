...
Let's try a "bad" model. Below we fit a model that doesn't take treat into account.

```{r}
lmmX <- lmer(wt ~ weeks + (1|subject), data = ratdrink)
```

Simulate wt using the model and overlay the model generated distributions on top of the observed distribution. 
...
It doesn't look horrible, but in the 100 - 200 range the observed data does not seem consistent with the model generated data.

Bootstrapping involves _resampling_ our data with replacement, calculating some statistic, repeating many times, and using the results to approximate a confidence interval. One reason we might want to do this for mixed-effect models is that the usual `predict` method does not compute standard errors.

Here's a quick demo of bootstrapping using toy data. Say we want a bootstrap confidence interval.
...
3. repeat steps 2 and 3 many times to get "bootstrap replicates". We can do that with the `replicate()` function.

```{r}
B <- replicate(n = 1000, expr = {mean(sample(x, replace = TRUE))})
```

4. Use bootstrap replicates to get a confidence interval using quantiles.

```{r}
quantile(B, probs = c(0.025, 0.975))
```

To do this with an `lmer()` model we use the `bootMer()` function.

Let's say we want to calculate a bootstrap confidence interval on the difference in means between thiouracil and thyroxine at week 4 using our `lmm3` model.
