...
- mean weight at week 0 for control group is about 59
...
We have many options when building a mixed-effect model. In addition to what predictors to include and whether they should interact, we get decide how many random effects to allow. The previous model allowed for one random effect on the intercept. 
...
In both cases it seems like the random effect for weeks is justified. 

In this updated model the intercept and weeks random effects are correlated. We can extract just the random effect variance estimates with the `VarCorr()` function,

```{r}
VarCorr(lmm3)
```

If we like we can fit a model with no correlation between random effects. To do this we use two pipes in the random effects formula: `(weeks || subject)`
...
Let's say we're happy with model `lmm3`. It's good enough. How can we use that model to make comparisons between treatments? How can we compare trends over time between treatments?

A package that is useful for this is the **emmeans** package.
...
Compare thiouracil and thyroxine at week 4.
...
```{r}
head(b.out$t)
quantile(b.out$t, probs = c(0.025, 0.5, 0.975))
```
                 

Another approach is to plot observed versus fitted values by subject. This is sometimes called a "calibration plot." A good model should return values with close to a 1:1 correspondence. It looks like some are ok, but others curve around the line.
