...
Next we generate an independent predictor variable, which is simply the numbers 1 - 10. We use the `rep()` to replicate the vector 1:10 seven times.
...
- for any subject, or 
- for a particular subject, such as subject 4 (id = 4)?
...
To make a _prediction for a specific subject_, we drop the `re.form=NA` argument and include `id` in our new data frame. Predict expected y when x = 3 for _for subject 4 (id = 4)_.

```{r}
predict(me1, newdata = data.frame(x = 3, id = 4))
```
...
To incorporate the additional uncertainty due to each subject's random effect, we set `type = "random"` (for "random effects") and add "id" to the `terms` argument. Now we have confidence ribbons for each subject.
...
4. Now fit a model that assumes the subject exerts a random effect on _both_ the intercept and slope, using the formula `y ~ x + (x|id)`. Call the model `me4`.
...
1. Let's fit a model with an interaction between treat and weeks, and let's leave the intercept conditional on subject. Add `treat:weeks` to the model specified in lmm1. Call the model `lmm2`
...
When comparing mixed-effect models it's best to either...
...
1. Update model lmm4 with a non-linear effect on weeks using `ns(weeks, 2)`. This is a second degree natural spline. Call the model `lmm5`.
...
Let's try `lmm5`. Recall the `lmm5` model:

```{r}
library(splines)
lmm5 <- lmer(wt ~ ns(weeks, 2) * treat + (1 | subject) + 
...
Compare the 95% bootstrap confidence interval to the 95% confidence interval returned by {emmeans} which uses something called the "Kenward-Roger approximation". The bootstrap CI is tighter compared to the approximated CI [-59.9, -19.7].
...
Register for the Research Data Services newsletter: https://library.virginia.edu/data/newsletters
...
So why is ML even available to use as an estimator if it returns biased estimators? It turns out that REML is not a suitable estimator when comparing models that differ _only in their fixed effects_. 
...
1. m: the model with just the random effect we're testing (weeks)
